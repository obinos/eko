...
<script>
    $(document).ready(function() {
        ...
        $("#form_edit").validate({
            rules: {
                ...
                phone: {
                    ...
                    number: true,
                    remote: {
                        url: "<?= base_url('user/check_user/telegram'); ?>",
                        type: "post",
                        data: {
                            id: function() {
                                return $("#id").val();
            ...
            messages: {
                ...
                phone: {
                    ...
                    remote: "no whatsapp sudah terdaftar."
                }
            },
            ...
            submitHandler: function() {
                ...
                $.ajax({
                    type: "POST",
                    url: '<?= base_url('user/update_telegram/') ?>',
                    data: {
                        data: jsonString
                    },
                    success: function(response) {
                        ...
                        if (result.status == 'success') {
                            if (result.action == 'Edit') {
                                ...
                                <tr id="` + result._id + `">
                                    <td>0</td>
                                    <td class="id_telegram"></td>
                                    <td class="username">` + result.username + `</td>
                                    <td class="phone">` + result.phone + `</td>
                                    <td class="is_active">` + result.is_active1 + `</td>
                                    <td class="role">` + result.role1 + `</td>
                                    <td class="created_at"><span class="pace-inactive">` + result.created_at1 + `</span>` + result.created_at2 + `</td>
                                    <td>
                                        <div class='btn-group btn-group-sm'>
                                            <a class='btn btn-warning btn_edit' data-tooltip="tooltip" data-placement="top" data-toggle="modal" data-id="` + result._id + `" title="Edit Data" href='#modal-form'>
                                                <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
                                                    <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456l-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                                    <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z" />
                                                </svg>
        ...
        $('.btn_edit').on('click', function(e) {
            ...
            $.ajax({
                type: "POST",
                url: '<?= base_url('user/get_telegram') ?>',
                data: {
                    data: id
                },
                success: function(response) {
        ...
        $('#datatable').on('click', '.del-button', function(e) {
            ...
            Swal({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: '<?= base_url('user/delete_telegram') ?>',
                        data: {
                            data: id
                        },
                        success: function(response) {
