...
<div class="wrapper wrapper-content animated fadeInRight">
    ...
    <div class="row d-flex justify-content-center">
        <?php foreach ($rak as $k => $val) {
            $theme = $k == 'rak-1' ? 'primary-element' : ($k == 'rak-2' ? 'warning-element' : 'danger-element');
            $bg = $k == 'rak-1' ? 'bg-primary' : ($k == 'rak-2' ? 'bg-warning' : 'bg-danger'); ?>
            <div class="col-lg-4 px-1">
                <div class="ibox mb-3">
                    ...
                    <div class="ibox-content p-3">
                        <ul class="sortable-list connectList agile-list" id="<?= $k ?>">
                            <?php $no = 1;
                            foreach ($val as $cou) { ?>
                                <li class="theme <?= $theme ?>" id="<?= $cou->_id ?>">
                                    <div class="row justify-content-center d-flex align-items-center">
                                        ...
                                        <div class="col-3">
                                            <h4 class="m-0 time" data-nominal="<?= $cou->nominal ?>"><?= $cou->time ?></h4>
                                        </div>
                                        <div class="col-3">
                                            <h4 class="m-0 order"><?= $cou->order ?></h4>
                                        </div>
                                        <div class="col-12">
                                            <?php $num = 1;
                                            $catatan = $cou->cat ? $cou->cat : $cou->note;
                                            foreach ($catatan as $cat) {
                                                $key = array_search($cat, $cou->note);
                                                $checked = $key !== false ? 'checked' : '';
                                                $numcheck = $num++; ?>
                                                <div class="form-check abc-checkbox abc-checkbox-success">
                                                    <input class="cat" type="hidden" value="<?= $cat ?>">
                                                    <input class="form-check-input note" type="checkbox" value="<?= $cat ?>" id="<?= $cou->_id . $numcheck ?>" <?= $checked ?>>
